...
public class JoinRevListDAO {
	...
	public int getReservationUpdate(JoinRevList joinrevlist) {
		...
		try {
			con = DBUtil.getConnection();
			if (con != null) {
				System.out.println("JoinRevListDAO.getReservationUpdate : DB 연결 성공");
			} else {
				System.out.println("JoinRevListDAO.getReservationUpdate : DB 연결 실패");
			}
			String query = "update scheduletbl set RoomName = ? , startTime = ?, endTime = ?, personNum = ? where userID = ?";
			pstmt = con.prepareStatement(query);

			pstmt.setString(1, joinrevlist.getJRoomName());
			pstmt.setInt(2, joinrevlist.getJStartTime());
			pstmt.setInt(3, joinrevlist.getJEndTime());
			pstmt.setInt(4, joinrevlist.getJPersonNum());
			pstmt.setString(5, joinrevlist.getJUserID());
			...
			alert.setHeaderText(joinrevlist.getJName() + "님 예약 수정 실패");
			alert.showAndWait();
		} finally {
			try {
				if (pstmt != null)
